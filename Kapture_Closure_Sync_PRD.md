...
| **Version** — v0.3 · 18 Sep 2026 | **Consulted — Quality OS** — Akhil | **Consulted — Support/Ops** — [TBD — not asked] | **Consulted — TAS eng** — [TBD — not asked] |
...
| AC | Given / When / Then | Verifies | Status |
|---|---|---|---|
| AC-R3-1 | **Given** ticket 1786508079949000 with the CSP's app installed and the complaint open, **When** the closure from AC-R1-1 is accepted, **Then** a push notification reaches the CSP's device within C-03 [TBD — not asked]. | R3a | OPEN |
...
| ID | Story | MUST | MUST NOT |
|---|---|---|---|
| R4 | As a CSP, I want tapping ठीक है to clear the card from my list, so that my feed holds only work I still owe. | **(a)** On ठीक है, move the card to the archive. **(b)** Accept the tap from the CSP who owns the card or from the technician assigned to it. **(c)** Until the tap, keep the card in the feed. | Archive the card before it is acknowledged; reopen or re-activate the complaint on the tap; accept the tap from anyone other than the card's CSP or its assigned technician. |
...
| T6 | COMPLETED (unacknowledged) | Time passes, no acknowledgement | — | COMPLETED (unacknowledged) | `[CLARIFY: whether the card stays in the feed indefinitely, given C-01 moves it to the archive at 7 days on age alone — see AC-R4-4.]` |
...
| AC-FAIL-1 | **Given** a Kapture closure accepted at 09:20:23, **When** C-04 [TBD — not asked] has elapsed and the complaint is still not CLOSED, **Then** the closure is visible to Support/Ops as undelivered, carrying the ticket id and the agent's closure instant, so that the complaint can be closed against that instant rather than against the CSP. | R1a · C-04 · G1 | OPEN |
...
| AC-BV-1 | **Given** the card resolved at 2026-08-12T09:20:23 and never acknowledged, **When** the CSP opens the feed at 2026-08-19T09:20:22 — one second inside C-01 (7 days) — **Then** `[CLARIFY: is the card in the feed? This is the boundary AC-R4-4 turns on.]` | C-01 · R4c | OPEN |
| AC-BV-2 | **Given** the same unacknowledged card, **When** the CSP opens the feed at 2026-08-19T09:20:24 — one second past C-01 — **Then** `[CLARIFY: feed or archive?]` | C-01 · R4c | OPEN |
| AC-BV-3 | **Given** the same unacknowledged card, **When** the CSP opens the archive at 2026-09-11T09:20:24 — one second past C-02 (30 days) — **Then** `[CLARIFY: the card is gone from both surfaces; is an unacknowledged card allowed to disappear unacknowledged?]` | C-02 · R4c | OPEN |
...
|---|---|---|---|---|
| C-01 | Gateway v2 inline feed window — how long a resolved card stays in the feed before it is reachable only in the archive | 7 days (existing) | Fixed | — |
| C-02 | TAS terminal retention — how long a resolved card is retained at all | 30 days (existing) | Fixed | — |
| C-03 | Kapture closure accepted → push notification reaches the CSP | [TBD — not asked] | [TBD — not asked] | — |
| C-04 | Kapture closure accepted → complaint CLOSED, or the closure is raised to Support/Ops | [TBD — not asked] | [TBD — not asked] | — |

**Interaction note (C-01 × C-02).** These are existing platform windows, not values this feature introduces, and they are the reason AC-R4-4 and T6 are OPEN: between C-01 and C-02 the card is in the archive rather than the feed, and after C-02 it is gone — so an unacknowledged card cannot in fact remain in the feed indefinitely without changing both.
...
| §5 · C-03 | How fast must the push notification reach the CSP after the closure is accepted, and is that fixed or configurable? |
