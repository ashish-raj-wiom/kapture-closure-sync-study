...
| T6 | COMPLETED (unacknowledged) | Time passes, no acknowledgement | — | COMPLETED (unacknowledged) | `[CLARIFY: whether the card stays in the feed indefinitely, given the gateway moves it to the archive at 7 days on age alone — see AC-R4-4 and §6 TAS aggregation.]` |
...
| TAS aggregation (feed) | Card ordering and archival | `aggregation/service/AggregationEngine.java:40,53,152`; `aggregation/config/TasParameters.java:44-75` | The v2 feed sorts on `latest_attention_at` DESC and nothing else, so raising the update *is* the rise — no ordering rule is needed or possible. Terminal cards already render greyed with no actionable dot. **Two existing windows govern how long a resolved card survives, and this feature sets neither:** the gateway's v2 inline feed window is **7 days**, after which the card is reachable only in the archive, and `terminalRetention` is **30 days**, after which it is gone from both. They are why R4c is OPEN — a card cannot stay in the feed until acknowledged while age alone removes it · AC-R2-4 · AC-R4-4 · AC-BV-1 · AC-BV-2 · AC-BV-3 |
...
| Record an acknowledgement against a card and move it to the archive on that acknowledgement rather than on age. | T4 · R4a · §6 TAS aggregation |
