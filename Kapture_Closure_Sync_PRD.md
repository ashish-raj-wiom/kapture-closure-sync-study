...
| ID | Story | MUST | MUST NOT |
|---|---|---|---|
| R4 | As a CSP, I want the card to stay live until I acknowledge it — even though the fault is already fixed — so that I see Wiom acted on my customer's problem before I did, and my feed then holds only work I still owe. | **(a)** On ठीक है, move the card to the archive. **(b)** Accept the tap from the CSP who owns the card or from the technician assigned to it. **(c)** Until the tap, keep the card in the feed and **active**: it renders as live work carrying its actionable state, not as a finished card, so that the CSP sees Wiom acted rather than finding the job quietly gone. **(d)** Archive the card without a tap once it has gone C-03 unacknowledged, so an unacknowledged card cannot pile up in the feed for ever. | Archive the card before it is acknowledged and before C-03 has elapsed; render it as finished, greyed or unactionable while it still awaits the tap; reopen or re-activate the complaint on the tap; accept the tap from anyone other than the card's CSP or its assigned technician. |
...
| AC-R4-4 | **Given** the card resolved at 09:20:23 on 12 Aug and never acknowledged, **When** the CSP opens the feed on 20 Aug — eight days later, past C-03 (7 days) — **Then** the card is no longer in the feed and is retrievable in the archive, still unacknowledged. | R4d · C-03 | Settled |
...
| T6 | COMPLETED (unacknowledged) | C-03 elapses with no acknowledgement | — | COMPLETED (archived) | Card leaves the feed unacknowledged and is retrievable in the archive (R4d); complaint untouched. |
...
| AC-BV-1 | **Given** the card resolved at 2026-08-12T09:20:23 and never acknowledged, **When** the CSP opens the feed at 2026-08-19T09:20:22 — one second inside C-03 (7 days) — **Then** the card is in the feed, active, still carrying ठीक है. | R4c · C-03 | Settled |
| AC-BV-2 | **Given** the same unacknowledged card, **When** the CSP opens the feed at 2026-08-19T09:20:24 — one second past C-03 — **Then** the card is in the archive, not the feed. | R4d · C-03 | Settled |
| AC-BV-3 | **Given** the same unacknowledged card, **When** the CSP opens the archive at 2026-09-11T09:20:24 — one second past TAS's 30-day terminal retention — **Then** the card is gone from both feed and archive, exactly as an acknowledged card of the same age would be. | R4d | Settled |
...
| C-03 | How long a resolved-unacknowledged card stays in the feed before it archives itself | 7 days | Configurable | 1–30 days ⚠️ *AI GENERATED — review* · TAS / Product Ops ⚠️ *AI GENERATED — review* · an unacknowledged card accumulates, and the tail is steep — the median CSP sees 2 a month, the heaviest 240 — so the window is tuned down if feeds fill and up if CSPs lose cards before they act ⚠️ *AI GENERATED — review* |
...
| Resolved-unacknowledged ⚠️ *AI GENERATED — review* | A card whose complaint and candidate are resolved, which stays active in the CSP's feed carrying one action — ठीक है — until the CSP or their assigned technician taps it, or C-03 elapses. It is deliberately not shown as finished: the CSP is meant to see that Wiom closed the fault, not to find the card silently gone. | TAS |
...
| Record an acknowledgement against a card, archiving it on that acknowledgement or on C-03, whichever comes first. | T4 · T6 · R4a · R4d · C-03 |
...
| §5 · C-03 | The range (1–30 days), the owner (TAS / Product Ops) and the stated reason for making the window configurable | PM gave the value (7 days) and that it is a config; the three supporting cells are mine. Range is bounded above by TAS's 30-day retention, below which a card would outlive the archive that holds it. |
