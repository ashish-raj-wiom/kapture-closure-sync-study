...
ev AS (SELECT DISTINCT l.TASK_ID, DATEADD(MINUTE,330,l.ADDED_TIME) ts,
              ...
              TRY_PARSE_JSON(l.DATA):status::VARCHAR st
...
kf AS (SELECT TASK_ID FROM pt WHERE fka IS NOT NULL AND (fca IS NULL OR fka < fca))
SELECT t.SUB_STATUS, t.RESOLUTION_TYPE, COUNT(*) tickets,
       ROUND(100.0*COUNT(*)/SUM(COUNT(*)) OVER (),2) pct
FROM PROD_DB.DYNAMODB_READ.TICKETS t JOIN kf ON kf.TASK_ID = t.TICKET_ID::VARCHAR
